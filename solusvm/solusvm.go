...
var (
	ErrAuthenticationKeysNotFound = errors.New("[id, key] not found. Did you send it ?")
	ErrActionNotFound             = errors.New("[action] not found. Did you send it ?")
)

// A Client manages communication with the Solusvm API.
type Client struct {
	// HTTP client used to communicate with the API.
	client *http.Client

	// Base URL for API requests.  Defaults to https://www.yourdomain.com/api/admin/, but can be
	// set to a domain endpoint to use with your billing at your enterprise.  BaseURL should
	// always be specified with a trailing slash.
	BaseURL *url.URL

	// API endpoint suffix used when communicating with the Solusvm API.
	ApiEndSux string
...
// NewClient returns a new Solusvm API client.  If a nil httpClient is
...
func (c *Client) NewWRequest(dat map[string]string, action string) (*WRequest, error) {
		rel, err := url.Parse(dat[apiEndSux])
		if err != nil {
			return nil, err
		}
...
// Response is a Solusvm API response.  This wraps the standard http.Response
// returned from Solusvm and provides convenient access to things like
// pagination links.
...
func (c *Client) Do(req WRequest, v interface{}) (*Response, error) {
 ...
 //to avoid ssl verification (import
 tr := &http.Transport{
...
func do(c *Client, p Params, a interface{}) (*Response, error) {
	...
	fmt.Printf("\n NewWRequest  req data -  %#v ",req.data)
...
func (c *Client) auth(w WRequest) (*url.URL, error) {
	if !(w.howzzat(USERNAME) && w.howzzat(PASSWORD)) {
...
Solusvm API docs: http://developer.Solusvm.com/v3/#client-errors
*/
...
// parseBoolResponse determines the boolean result from a Solusvm API response.
// Several Solusvm API methods return boolean responses indicated by the HTTP
